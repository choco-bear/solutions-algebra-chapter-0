...
\problem{
    Let $\cat{C}$ be a category with products. Find a reasonable candidate for the universal property that the product $A \times B \times C$ of \emph{three} objects of $\cat{C}$ ought to satisfy, and prove that both $(A \times B) \times C$ and $A \times (B \times C)$ satisfy thsi universal property. Deduce that $(A \times B) \times C$ and $A \times (B \times C)$ are necessarily isomorphic.
}{}
...
\problem{
    ...
    \begin{itemize}[label=$\bullet$]
        \item Use the universal property for quotients (\S5.3) to establish that there are functions $(A \tiems B)/\sim \to A/\sim_A$, $(A \tiems B)/\sim \to B/\sim_B$.
...
\problem{
    $\neg$ Define the notions of \emph{fibered products} and \emph{fibered coproducts}, as terminal objects of the categories $\cat{C}_{\alpha,\beta}$, $\cat{C}^{\alpha,\beta}$ considered in Example 3.10 (cf. also Exercise 3.11), by stating carefully the corresponding universla properties.
