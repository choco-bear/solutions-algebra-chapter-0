...
\problem{ %9.1
    ...
    \begin{itemize}
        \item Let $M \in \mathrm{O}_n(\RR)$.
        Then since $\left< M\mathbf{x} , M\mathbf{y} \right> = \transpose{\left(M\mathbf{y}\right)}M\mathbf{x} = \transpose{\mathbf{y}}\mathbf{x} = \left< \mathbf{x} , \mathbf{y} \right>$, $\norm{\mathbf{x}} = \norm{M\mathbf{x}}$ and the action by $M$ preserves the angle between vectors.
        
        \item Since $\SU{2} = \left\{\mat{\alpha & -\bar{\beta} \\ \beta & \bar{\alpha}} \mid \alpha, \beta \in \CC, \abs{\alpha}^2 + \abs{\beta}^2 = 1\right\}$, we can consider each element of $\SU{2}$ as a unit quaternion $q = \alpha + \alpha \mathbf{j}$.
        Let us now identify $\RR^3$ with the span of $\mathbf{i}$, $\mathbf{j}$, $\mathbf{k}$.
        Then for any $\mathbf{v} \in \RR^3$ and any unit quaternions $q \in \HH$, $qvq^{-1} \in \RR^3$.
        Hence, we can say that there is an interesting action of $\SU{2}$ on $\RR^3$.
    \end{itemize}
}
...
\problem{ %9.2
    ...
    It is quite obvious that $\left< \mat{1 & 0 \\ 0 & -1}, \mat{0 & 1 \\ -1 & 0} \right>$ is isomorphic to $D_8$.
    Hence, we can easily find an action of $D_8$ on $\RR^2$ by applying left action of $\left< \mat{1 & 0 \\ 0 & -1}, \mat{0 & 1 \\ -1 & 0} \right>$ on $\RR^2$. 
}
...
\problem{ %9.3
    ...
    \begin{itemize}
        \item Let $g,h,k \in G^\circ$.
        Then, $\left( g \bullet h \right) \bullet k = k \cdot \left( h \cdot g \right) = \left( k \cdot h \right) \cdot g = g \bullet \left( h \bullet k \right)$ and thus the associativity of $G^\circ$ is verified.
        Also, the existences of identity and inverse are quite obvious by the definition.
        Therefore, $G^\circ$ is indeed a group.

        \item Let a map $\varphi : G^\circ \to G$ be defined as $g \mapsto g$.
        Then $\varphi$ is indeed a bijection, and thus it is an isomorphism if and only if $g \bullet h = \varphi(g \bullet h) = \varphi(g) \cdot \varphi(h) = g \cdot h = h \bullet g$ for any $g,h \in G$.

        \item Define $\varphi : G \to G^\circ$ as $g \mapsto g^{-1}$.
        Then, $\varphi(g \cdot h) = \left(g \cdot h\right)^{-1} = h^{-1} \cdot g^{-1} = g^{-1} \bullet h^{-1} = \varphi(g) \bullet \varphi(h)$.
        Also, $\varphi$ is indeed a bijection, and thus $\varphi$ is an isomorphism.
        Therefore, $G^\circ \cong G$.

        \item Let $\varphi : A \times G \to G$ be a map representing a right-action of $G$ on a set $A$.
        That means, for any $g,h \in G$ and any $a \in A$, $\varphi(a, g \cdot h) = \varphi(\varphi(a,g),h)$.
        Now take a left-action $\sigma$ of $G^\circ$ on $A$ defined as $\sigma(g)(a) = \varphi(a,g)$.
        Then we can easily verify that $\varphi$ and $\sigma$ are actually representing the same instance.

        \item By the right above result, and by the second result of this exercise, it is quite obvious.

        \item Let $\varphi : A \times G \to G$ be a map representing a right-action of $G$ on a set $A$.
        Then, a left-action $\sigma$ of $G$ on the set $A$ defined as $\sigma(g)(a) = \varphi(a,g^{-1})$ has the same structure with $\varphi$.
    \end{itemize}
}
...
\problem{ %9.4
    ...
    A map $\varphi : G \times G \to G$ defined as $(a,g) \mapsto g^{-1}a$ is a right-action of $G$ on itself.
}

\problem{ %9.5
    Prove that the action by left-multiplication of a group on itself is free.
}{
    Let $G$ be a group and $ga = a$ for some $g,a \in G$.
    Then, $g$ is an identity element, and thus the action by left-multiplication of $G$ on itself is free.
}
...
\problem{ %9.6
    ...
    For any elements $x,y \in O$, since $O$ is an orbit, there is an element $a \in O$ and some $g,h \in G$ such that $x = ga$ and $y = ha$.
    Hence, by the definition of group action, we can conclude that $x = gh^{-1}y$.
    Therefore, the induced action of $G$ on $O$ is transitive.
}

\problem{ %9.7
    Prove that stabilizers are indeed subgroups.
}{
    Let $G$ be a group and $A$ be a $G$-$\Set$.
    Now take $a \in A$ and consider $\Stab{G}{a} = \{ g \in G \mid ga = a \}$.
    If $g,h \in \Stab{G}{a}$, then $\left(gh\right)a = g\left(ha\right) = ga = a$ and $g^{-1}a = g^{-1}\left(ga\right) = \left(g^{-1}g\right)a = a$, which means $gh, g^{-1} \in \Stab{G}{a}$.
    Hence, $\Stab{G}{a}$ is a subgroup of $G$.
}
...
\problem{ %9.8
    ...
    To verify that $G$-$\Set$ is a category, we have to check the associativity of the composition of morphisms and the existence of identity morphisms.

    First of all, we should define the composition of morphisms in $G$-$\Set$.
    Consider three objects $\left( \rho, A \right)$, $\left( \sigma, B \right)$, $\left( \tau, C \right)$ of $G$-$\Set$, and two equivariant set-functions $\varphi : A \to B$ and $\psi : B \to C$.
    Then we can easily verify that $\psi \circ \varphi$ is indeed an equivariant set-function since $\left( \id{G} \times \psi \right) \circ \left( \id{G} \times \varphi \right) = \id{G} \times \left( \psi \circ \varphi \right)$.
    Then by the associativity of the composition of set-functions, the composition of morphisms in $G$-$Set$ is also associative.

    Next, we should check if there are identity morphisms in $G$-$\Set$.
    Consider an object $\left( \rho, A \right)$ of $G$-$\Set$.
    Then, an identity function $\id{A} : A \to A$ is indeed an equivariant set-function.
    Hence, we can consider the identity function $\id{A}$ as the identity morphism $\one[\left( \rho, A \right)]$.

    Therefore, we can say that $G$-$\Set$ is indeed a category.
    Now, we should verify that the isomorphisms in $G$-$\Set$ are precisely the equivariant bijections.

    Let $\varphi : A \to B$ is an isomorphism between the objects $\left( \rho, A \right)$ and $\left( \sigma, B \right)$.
    Then there is a morphism $\psi : B \to A$ such that $\psi \circ \varphi = \one[\left( \rho, A \right)]$ $\varphi \circ \psi = \one[\left( \sigma, B \right)]$.
    Hence, $\psi$ is the inverse function of $\varphi$, and thus $\varphi$ is bijective.

    Now let $\varphi : A \to B$ is a bijective equivariant function.
    Then, considering the inverse function $\varphi^{-1} : B \to A$, we can easily verify that $g\varphi^{-1}(b) = \varphi^{-1}(gb)$ for any $g \in G$ and $b \in B$ since $\varphi(g\varphi^{-1}(b)) = g\varphi(\varphi^{-1}(b)) = gb = \varphi(\varphi^{-1}(gb))$.
    Hence, $\varphi$ is an isomorphism indeed.
    
    Therefore, the isomorphisms in $G$-$\Set$ are precisely the equivariant bijections.
}
...
\problem{ %9.9
    ...
    Let $\left\{\left( A_i, \sigma_i \right)\right\}_{i \in I}$ be a set of objects of $G$-$\Set$.
    Take any object $\left( B, \tau \right)$ of $G$-$\Set$.
    Take the projections $\pi_i : \left( a_j \right)_{j \in I} \mapsto a_i$.
    Now let $\varphi_i : B \to A_i$ be given.
    Then $\varphi : B \to \prod_{i \in I} A_i$ defined as $b \mapsto \left( \varphi_i(b) \right)_{i \in I}$ is the product morphism.
    \begin{cd}[row sep=1.5em, column sep=3em]
        B \ar[rd, "\varphi", dashed, swap] \ar[rr, "\varphi_i"] \&\& A_i \\
        \& \prod_{i \in I} A_i \ar[ur, "\pi_i", swap]
    \end{cd}

    Every $G$-set is a coproduct of orbits indeed, and thus every $G$-set is a coproduct of the objects of the type of $G/H$.
}
...
\problem{ %9.10
    ...
    It is clear that $H \backslash G$ is isomorphic to $G^\circ / H^\circ$ in $\Set$ category.
    Thus, $\abs{H \backslash G} = \abs{G/H}$ since $G \cong G^\circ$ and $\varphi(H) = H^\circ$ where $\varphi$ is the canonical isomorphism.
}
...
\problem{ %9.11
    ...
    Let $\sigma(g) : p \to p$ be defined as $i \mapsto \varphi^{-1}\left(g\varphi(i)\right)$ where $\varphi : p \to G/H$ is a bijection.
    Then the action of $G$ on $G/H$ by left multiplication can be interpreted as $\sigma : G \to S_p$.
    It is clear that $G/\ker\sigma$ is isomorphic to a subgroup of $S_p$.
    Hence, the index of $\ker\sigma$ divides $p!$.
    
    Now let $g \in \ker \sigma$.
    Then, $\varphi^{-1}\left(g\varphi(i)\right) = i$ for any $i \in p$.
    This means that $g \in H$ since $\varphi$ is a bijection.
    Hence, $\ker \sigma \subseteq H$.
    Thus, the index of $\ker \sigma$ in $G$ is at least $p$.
    However, there is only $x = p$ such that $p \leq x$, $x \mid p!$, and the least prime divisor of $x$ is at least $p$.
    Therefore, $\ker \sigma = H$, and thus $H$ is normal.
}

\problem{ %9.12
    $\neg$ Generalize the result of \ref{prob:II.9.11}, as follows.
    Let $G$ be a group, and let $H \subseteq G$ be a subgroup of index $n$.
    Prove that $H$ contains a subgroup $K$ that is normal in $G$ and such that $\sgindex{G}{K}$ divides the gcd of $\abs{G}$ and $n!$.
    (In particular, $\sgindex{G}{K} \leq n!$.) \sqref{IV.2.23}
}{
    Consider an action of $G$ on $H$ by left-multiplication, and let $\sigma : G \to S_n$ be the interpretation of the action as the one in \ref{prob:II.9.11}.
    Now let $K = \ker \sigma$.
    Then $K \normaleq G$ and it is clear that $\ker \sigma \leq H$ and $\sgindex{G}{K} \mid n!$.
}
...
\problem{ %9.13
    ...
    We want to find a bijection $\varphi : G/H \to G/(gHg^{-1})$ such that the diagram
    \begin{cd}
        G \times G/H \ar[r, "\id{G} \times \varphi"] \ar[d, "\rho"] \& G \times G/(gHg^{-1}) \ar[d, "\rho"] \\
        G/H \ar[r, "\varphi"] \& G/(gHg^{-1})
    \end{cd}
    commutes where $\rho : (g,A) \mapsto gA$.

    Take $\varphi : aH \mapsto agHg^{-1}$.
    Then the well-definedness and the bijectivity of $\varphi$ is quite obvious.
    Also, since $\left(ab\right)gHg^{-1} = a\left(bgHg^{-1}\right)$, $\varphi$ makes the diagram commute.
    Therefore, $G/H$ and $G/gHg^{-1}$ are isomorphic in $G$-$\Set$.
}
...
\problembr{ %9.14
    ...
    This latter verification is traditionally carried out by cleverly exploiting an action.
    ...
    Let $\mat{a & b \\ c & d}(r) = \frac{ar+b}{cr+d}$.
    Then it is clear that $x(r) = (-x)(r)$ for any $x \in \SL{2}{\ZZ}$, and thus it is well-defined when $x \in \PSL{2}{\ZZ}$.
    Further,
    \[\begin{array}{rcl}
        \mat{a & b \\ c & d} \left( \mat{x & y \\ z & w} (r) \right) &\centered{=}[-1ex]& \mat{a & b \\ c & d} \left(\frac{xr+y}{zr+w}\right) \\
        &\centered{=}[-1ex]& \frac{a(xr+y)+b(zr+w)}{c(xr+y)+d(zr+w)} \\
        &\centered{=}[-1ex]& \frac{(ax+bz)r+(ay+bw)}{(cx+dz)r+(cy+dw)} \\
        &\centered{=}[-1ex]& \mat{ax+bz & ay+bw \\ cx+dz & cy+dw} (r) \\
        &\centered{=}[-1ex]& \left( \mat{a & b \\ c & d} \mat{x & y \\ z & w} \right) (r).
    \end{array}\]

    Hence, $\rho : \PSL{2}{\ZZ} \times \left(\RR - \QQ\right) \to \left(\RR - \QQ\right)$ defined as $\left( \mat{a & b \\ c & d}, r \right) \mapsto \frac{ar+b}{cr+d}$ makes the following diagram commute, where $\odot_\RR$ denotes the wheel algebra which extends $\RR$:
    \begin{cd}[row sep=4em]
        \& \centered{$\PSL{2}{\ZZ} \times \PSL{2}{\ZZ} \times \left(\RR - \QQ\right)$}[\widthof{$\left(\RR - \QQ\right)$}] \ar[dl, "\id{\PSL{2}{\ZZ}} \times \rho", swap] \ar[dr, "m_G \times \id{\RR}"] \\
        \PSL{2}{\ZZ} \times \left(\RR - \QQ\right) \ar[dr, "\rho", swap] \&\& \PSL{2}{\ZZ} \times \left(\RR - \QQ\right) \ar[dl, "\rho"] \\
        \& \RR - \QQ
    \end{cd}
...
\problem{ %9.15
    ...
    Considering the definition of Cayley graph, each vertex of Cayley graph represents each of the elements of $G$.
    Hence, the action of $G$ on corresponding Cayley graph has the same structure with the action of $G$ on itself defined by left-multiplication.
    Thus, the action is free obviously.
    Further, since the Cayley graph corresponding to a free group is always tree, every free group acts freely on a tree indeed.
}
...
\problem{ %9.16
    ...
    Let $G$ be a free group on a finite set, and $H$ be a subgroup of $G$.
    Then, since $G$ acts freely on a tree $T$, we can obtain an action of $H$ on $T$ induced by the free action of $G$ on $T$.
    Hence, by the given fact, that is, only free groups can act freely on a tree, $H$ is also free indeed.
    Therefore, every subgroup of a free group on a finite set is free.
}
...
\problem{ %9.17
    ...
    Let $\varphi_g : G \to G$ be defined as $h \mapsto hg$ for each $g \in G$.
    Now consider a function $\varphi : G \to \Aut{G\text{-}\Set}[G]$ defined as $g \mapsto \varphi_g$.
    Then since $\varphi(g_1)\varphi(g_2) = \varphi(g_2g_1)$, $\varphi$ can be considered a group-homomorphism from $\op{G}$ to $\Aut{G\text{-}\Set}[G]$.
    Hence, if $\varphi$ is bijective, then it is clear that $G \cong \Aut{G\text{-}\Set}[G]$.

    The injectivity is obvious so it suffices to show $\varphi$ is surjcetive.
    Let $\psi \in \Aut{G\text{-}\Set}[G]$.
    Then we can take a $g \in G$ such that $g = \psi(e)$.
    Since $\psi(h) = \psi(he) = h\psi(e) = hg$ for any $h \in G$, we can conclude that $\psi = \varphi_g$ and thus makes $\varphi$ be surjective.

    Therefore, $G \cong \Aut{G\text{-}\Set}[G]$.
}
...
\problem{ %9.18
    ...
    Let $A$ be the set of objects and $\Hom{x}{y} = \{ \left(g,x,y\right) \mid g \in G \land gx = y \}$, and let the composition interpret the multiplication in $G$.
    Then we can say that $\left(e,x,x\right)$ is the identity morphism of $x$.
    Then, we can easily verify that the structure made above is actually a groupoid.
}
...
\problembr{ %10.1
    ...
    (For the projection $\one \times G \to G$, what is left to prove is that the composition
    \[\one \times G \to G \to \one \times G\]
    ...
    \begin{cd}[shorten <>=0pt, column sep=3em]
        \left(G \times G\right) \times G \ar[d, "\cong", swap] \ar[r, "m \times \id{G}"] \& G \times G \ar[r, "m"] \& G \ar[d, no head, shift left=0.5] \ar[d, no head, shift right=0.5] \\
        G \times \left(G \times G\right) \ar[r, "\id{G} \times m"] \& G \times G \ar[r, "m"] \& \lefted{$G,$}[\widthof{$G$}]
    \end{cd}
    consider the commutative diagram induced by the universal properties of product objects, where the $\pi$'s and $\pi'$'s are the projection morphisms:
    \begin{cd}[column sep=7em]
        \left(G \times G\right) \times G \ar[dd, "\pi_G", swap] \ar[r, "\pi_{G \times G}"] \ar[ddr, "\adjustbox{scale=0.8}{$\left(\pi_2\pi_{G \times G}\right) \times \pi_G$}"{pos=0.65}, swap, dashed] \ar[ddrr, dashed, to path={(\tikztostart.north) .. controls ([yshift=7em,xshift=10em]\tikztostart.north) and ([xshift=8em,yshift=10em]\tikztotarget.east) .. ([yshift=1ex]\tikztotarget.east)}] \ar[rr, "\adjustbox{scale=0.7}{$\left(\pi_1\pi_{G \times G}\right) \times \left(\left(\pi_2\pi_{G \times G}\right) \times \pi_G\right)$}", bend left=40, phantom] \& G \times G \ar[d, "\pi_2", swap] \ar[r, "\pi_1"] \& G \\
        \& G \\
    ...
    in
    \begin{cd}[shorten <>=0pt, column sep=3em]
        \one \times G \ar[r, "e \times \id{G}"] \ar[dr, "\cong", swap] \& G \times G \ar[d, "m"] \\
        \& \lefted{$G,$}[\widthof{$G$}]
    \end{cd}
    ...
    \begin{cd}[row sep=0.8em, column sep=4em]
        \&\& \one \\
        G \ar[rru, "\epsilon", bend left] \ar[rrd, "\id{G}", swap, bend right] \ar[r, "\epsilon \times \id{G}", dashed] \& \one \times G \ar[ru, "\pi_1", swap] \ar[rd, "\pi_G"]\\
        \&\& G
    \end{cd}
    ...
    in
    \begin{cd}[shorten <>=0pt, column sep=3em]
        G \times \one \ar[r, "\id{G} \times e"] \ar[rd, "\cong", swap] \& G \times G \ar[d, "m"] \\
        \& G.
    \end{cd}
}
\clearpage
...
\problem{ %10.2
    $\vartriangleright$ Show that \emph{groups}, as defined in \S1.2, are `group objects in the category of sets'. \sqref{\S10.1}
}{
    Let $\left(G,\cdot\right)$ be a group, and define $m : G \times G \to G$ and $\iota : G \to G$ as $\left(g,h\right) \mapsto gh$ and $g \mapsto g^{-1}$, respectively.
    Then, the followings are commutative where $\Delta = \id{G} \times \id{G}$:
    \begin{cd}[shorten <>=0pt, column sep=3em]
        \left(G \times G\right) \times G \ar[d, "\cong", swap] \ar[r, "m \times \id{G}"] \& G \times G \ar[r, "m"] \& G \ar[d, no head, shift left=0.5] \ar[d, no head, shift right=0.5] \\
        G \times \left(G \times G\right) \ar[r, "\id{G} \times m"] \& G \times G \ar[r, "m"] \& G
    \end{cd}
    \begin{cd}[shorten <>=0pt, column sep=3em]
        \one \times G \ar[r, "e \times \id{G}"] \ar[rd, "\cong", swap] \& G \times G \ar[d, "m"] \& G \times \one \ar[r, "\id{G} \times e"] \ar[dr, "\cong", swap] \& G \times G \ar[d, "m"] \\
        \& G \&\& G
    \end{cd}
    \begin{cd}[shorten <>=0pt, column sep=3em]
        G \ar[r, "\Delta"] \ar[d] \& G \times G \ar[r, "\id{G} \times \iota"] \& G \times G \ar[d, "m"] \& G \ar[r, "\Delta"] \ar[d] \& G \times G \ar[r, "\iota \times \id{G}"] \& G \times G \ar[d, "m"] \\
        \one \ar[rr, "e"] \&\& G \& \one \ar[rr, "e"] \&\& G
    \end{cd}
    Hence, $G$ is a group object in the category $\Set$.
}

\problem{ %10.3
    Let $\left(G,\cdot\right)$ be a group, and suppose $\circ : G \times G \to G$ is a group homomorphism (w.r.t. $\cdot$) such that $\left(G,\circ\right)$ is \emph{also} a group.
    Prove that $\circ$ and $\cdot$ coincide.
    (Hint: First prove that the identity with respect to the two operations must be the same.)
}{
    Let $e \in G$ be the identity of $\left( G,\cdot \right)$.
    Then since $\circ : G \times G \to G$ is a group-homomorphism, $e \circ e = e$.
    Since $\left( G,\circ \right)$ is also a group, there is an inverse of $e$ with respect to $\circ$, and thus $e$ is the identity of $\left( G,\circ \right)$.

    Now, let $g_1, g_2 \in G$.
    Then, $g_1 \circ g_2 = \left( g_1 \circ e \right) \cdot \left( e \circ g_2 \right) = g_1 \cdot g_2$ since $\circ$ is a group homomorphism.
    Therefore, $\circ$ and $\cdot$ coincide.
}

\problem{ %10.4
    Prove that every \emph{abelian} group has exactly one structure of group object \emph{in the category} $\Ab$.
}{
    Let $G$ be an abelian group and $\iota : G \to G$ and $\circ : G \times G \to G$ be morophism of $\Ab$ and $G$, $\iota$, $\circ$, $e$ form a structure of group object in $\Ab$.
    
    Then by the result of \ref{prob:II.10.3}, the multiplication and $\circ$ coincide, and thus $\iota \left( g \right) = g^{-1}$ for all $g \in G$.
}

\problem{ %10.5
    By the previous exercise, a group object in $\Ab$ is nothing other than an abelian group.
    What is a group object in $\Grp$?
}{
    Let $G$, $\iota$, $\circ$ form a structure of group object in $\Grp$.
    Then since $\left( gh \right) \circ \iota \left( gh \right) = e$, $\iota \left( gh \right) = \iota \left( h \right) \circ \iota \left( g \right)$.
    Also, since $\iota$ is a group-homomorphism, $\iota \left( gh \right) = \iota \left( g \right) \iota \left( h \right)$.
    Thus, by the result of \ref{prob:II.10.3}, $\iota \left( g \right) \circ \iota \left( h \right) = \iota \left( h \right) \circ \iota \left( g \right)$, and therefore, group objects in $\Grp$ are also abelian.
}