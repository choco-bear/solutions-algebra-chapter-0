...
\problem{ %4.3
    ...
    It is obvious that the `only if' part holds since $\eqcl{1}_n$ is an element of $\ZZ/n\ZZ$ whose order $n$.
...
\problem{ %4.7
    ...
    Applying cancelation, we get $hg = gh$, which means $G$ is abelian.
}
...
\problem{ %4.10
    $\vartriangleright$ Let $p \neq q$ be odd prime integers; show that $(\ZZ/pq\ZZ)^*$ is not cyclic.
    (Hint: Use \ref{prob:II.4.9} to compute the order $N$ of $(\ZZ/pq\ZZ)^*$, and show that no element can have order $N$.) \sqref{\S4.3}
}{
    It is obvious that $(\ZZ/pq\ZZ)^*$ is a group of order $(p-1)(q-1)$, and by the Fermat's little theorem, $p \nmid a \implies a^{p-1} \equiv 1 \pmod{p}$ and $q \nmid a \implies a^{q-1} \equiv 1 \pmod{q}$.
    Thus, by the Chinese remainder theorem, $\gcd (a,pq) = 1$ implies that $a^{\frac{1}{2}(p-1)(q-1)} \equiv 1 \pmod{pq}$ since $2 \nmid p$ and $2 \nmid q$.
    Therefore, there is no element of order $(p-1)(q-1)$ and, hence, $(\ZZ/pq\ZZ)^*$ is not a cyclic group.
}
...
\problem{ %4.11
    ...
    Assume this fact, and prove that the multiplicative group $G = (\ZZ/p\ZZ)^*$ is cyclic.
    ...
    Suppose not, i.e., suppose that $(\ZZ/p\ZZ)^*$ is not cyclic.
    Then, there exists an element $g \in (\ZZ/p\ZZ)^*$ whose order $d < p-1$ is maximal.
    This means that $n^d = 1$ for any $n \in (\ZZ/p\ZZ)^*$ and this contradicts with the fact that the equation $n^d = 1$ has at most $d$ solutions in $\ZZ/p\ZZ$.
    Therefore, $(\ZZ/p\ZZ)^*$ is cyclic.
}

\problem{ %4.12
    $\neg$ \begin{itemize}
        \item Compute the order of $\eqcl{9}_{31}$ in the group $(\ZZ/31\ZZ)^*$.
        \item Does the equation $x^3 - 9 = 0$ have solutions in $\ZZ/31\ZZ$?
        (Hint: Plugging in all 31 elements of $\ZZ/31\ZZ$ is too laborious and will not teach you much. 
    ...
    Therefore, there is no solution for the equation $x^3 - 9 = 0$ in $\ZZ/31\ZZ$.
}

\problem{ %4.13
    $\neg$ Prove that $\Aut{\Grp}[\ZZ/2\ZZ \times \ZZ/2\ZZ] \cong S_3$. \sqref{IV.5.14}
}{
    In $\ZZ/2\ZZ \times \ZZ/2\ZZ$, adding any two of three elements except the identity gives the other one.
    Also, $2x$ is the identity for any elements $x \in \ZZ/2\ZZ \times \ZZ/2\ZZ$.
    Thus, an automorphism on $\ZZ/2\ZZ \times \ZZ/2\ZZ$ can be considered as a permutation among the three elements except the identity.
    Therefore, $\Aut{\Grp}\left(\ZZ/2\ZZ \times \ZZ/2\ZZ\right) \cong S_3$.
}
