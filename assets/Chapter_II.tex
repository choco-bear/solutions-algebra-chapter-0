...
\problem{ %6.11
    ...
    The reader may be tempted to conjecture that every finitely generated \emph{group} is a coproduct \emph{in} $\Grp$.
...
\problem{ %6.14
    $\vartriangleright$ If $m$ is a positive integer, denote by $\phi(m)$ the number of positive integers $r < m$ that are \emph{relatively prime} to $m$ (that is, for which the $\gcd$ of $r$ and $m$ is $1$); this is called \emph{Euler's $\phi$-(or `totient') function}.
